...
function render(state) {
  ...
  return state
      .map((todo, index) => {
          ...
          const upArrow = index == 0 ? 'fa-stop' : 'fa-arrow-up'
          const upButtonClass = index == 0 ? '' : 'go-up'
          const downArrow = index == len - 1 ? 'fa-stop' : 'fa-arrow-down'
          const downButtonClass = index == len - 1 ? '' : 'go-down'
          ...
          const deadDate = document.createElement('td')
          deadDate.setAttribute('class', strikeString)
          deadDate.innerText = (todo.deadline.getMonth() + 1) + "/" + todo.deadline.getDate() + "/"+(todo.deadline.getYear()+1900)
...
$('#bin').on('click', function removeTodos() {
